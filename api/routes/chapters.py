...
chapters_bp = Blueprint('chapters', url_prefix='/chapters')
...
async def generate_chapters(request: Request):
    ...
    """
    # Get user_id from the decorator via Sanic's request.ctx
    user_id = getattr(request.ctx, 'current_user_id', None)
    if not user_id:
        logging.error("User ID not found in request.ctx within generate_chapters route.")
